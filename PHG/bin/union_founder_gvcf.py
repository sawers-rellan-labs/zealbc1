...
Records ONLY at the union sites; the donor's state_bayes decides the record:
  1  (own ALT, or gap ALT at posterior >= cut-off) -> alt record        REF ALT,<NON_REF>  GT:AD:DP:PL 1:0,d,0:d:90,90,0
  0  (gap REF: tier ref from the donor's own reads) -> 1-bp reference record  REF <NON_REF> END=pos  GT:AD:DP:PL 0:d,0:d:0,90,90
  NA (missing), and positions with two union alleles (multi-allelic) -> no record
Record format = pilot_step5_donor_gvcf.py's, so PHG loads it like the earlier founders.
Also writes <out>.alt.tsv (chrom pos ref alt) for the pseudo-assembly (bcftools consensus).
Usage: union_founder_gvcf.py --dhd dhd_bayes_chr10.tsv.gz --donor Zx.0570_P2 --fai B73.fa.fai --out D.g.vcf [--chrom chr10] [--depth 8] [--name NAME]"""
import argparse, gzip, collections
ap = argparse.ArgumentParser()
ap.add_argument('--dhd', required=True); ap.add_argument('--donor', required=True)
...
print(f"[union_founder] {NAME}: union positions {len(state)} | ALT records {n['alt']} | REF records {n['ref_blocks']} | missing (no record) {len(miss)}")
