...
a <- commandArgs(TRUE); rt_f <- a[1]; old <- a[2]; new <- a[3]; DN <- a[4]; ttl <- a[5]; out <- a[6]
...
calls <- rbind(phg(old, "PHG old reads (bwa, unfiltered)"), phg(new, "PHG MAPQ20 reads (minibwa CRAM)"))
if (!is.na(rt_f) && rt_f != "NA") { rt <- fread(rt_f)[chr == 10, .(name, chr = 10L, start_bp, end_bp, state, method = "RTIGER poolseq")]; calls <- rbind(rt, calls) }
