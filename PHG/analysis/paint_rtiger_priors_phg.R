...
# with the flanking ancestry, as in paint_callers_introg.R. Lines ordered by RTIGER TEO fraction; label = line (NIL id).
...
r3 <- rt(R3, "RTIGER design BC2S3"); r2 <- rt(R2, "RTIGER design BC2S2")
...
lb <- fread(LAB, header = FALSE, select = 1:2, col.names = c("sample", "nil")); m <- setNames(paste0(lb$sample, " (", lb$nil, ")"), lb$sample)
rel <- function(v) ifelse(v %in% names(m), m[v], v)
ordn <- rel(teo[order(-f, name)]$name); d[, name := factor(rel(name), levels = ordn)]
d[, method := factor(method, levels = c("PHG", "RTIGER design BC2S3", "RTIGER design BC2S2"))]
