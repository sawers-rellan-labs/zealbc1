...
a <- commandArgs(TRUE); R3 <- a[1]; PPAR <- a[2]; PHV <- a[3]; FN <- a[4]; LAB <- a[5]; out <- a[6]; TTL <- a[7]; BB <- if (length(a) >= 8 && a[8] != "NA") a[8] else NA; COV <- if (length(a) >= 9 && a[9] != "NA") a[9] else NA; FS <- if (length(a) >= 10) a[10] else NA; CHRLEN <- 152435371L
...
fs <- if (!is.na(FS)) { x <- fread(FS); lab <- sub("^fsfhap_", "fsfhap ", x$source[1]); x[chr == 10, .(name, chr = 10L, start_bp, end_bp, state, method = lab)] } else NULL
...
lanes <- c("RTIGER", if (!is.null(fs)) unique(fs$method), if (!is.null(bb)) unique(bb$method), "PHG"); d[, method := factor(method, levels = lanes)]
