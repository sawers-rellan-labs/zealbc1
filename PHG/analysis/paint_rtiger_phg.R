...
a <- commandArgs(TRUE); R3 <- a[1]; PPAR <- a[2]; PHV <- a[3]; FN <- a[4]; LAB <- a[5]; out <- a[6]; TTL <- a[7]; CHRLEN <- 152435371L
...
d[, method := factor(method, levels = c("RTIGER", "PHG"))]
...
ggsave(out, p, width = 14, height = paint_height(length(ordn), 2), dpi = 150, limitsize = FALSE); cat("wrote", out, "\n")
