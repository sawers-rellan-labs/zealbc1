...
# Usage: paint_rtiger_phg.R <rtiger.csv> <phg_parents_dir> <phg_hvcf_dir> <founder_name> <nil_labels.tsv> <out.png> <title> [bbnil.csv | NA] [coverage.tsv (SAMPLE, MEAN_COVERAGE) | NA] [lowcopy BED: PHG no-call + shared-haplotype ranges left blank]
...
a <- commandArgs(TRUE); R3 <- a[1]; PPAR <- a[2]; PHV <- a[3]; FN <- a[4]; LAB <- a[5]; out <- a[6]; TTL <- a[7]; BB <- if (length(a) >= 8 && a[8] != "NA") a[8] else NA; COV <- if (length(a) >= 9 && a[9] != "NA") a[9] else NA; NOFILL <- if (length(a) >= 10) a[10] else NA; CHRLEN <- 152435371L
