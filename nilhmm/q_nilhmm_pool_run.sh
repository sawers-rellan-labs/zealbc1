...
#   POOLS=4E,4F,4G sbatch --export=ALL q_nilhmm_pool_run.sh        (default POOLS=4E,4F,4G)
#   POOLS=... NF_RESUME=<sessionId> sbatch --export=ALL ...          to resume a session that completed DEMUX
# Set the variables in the ENVIRONMENT (before sbatch): Slurm splits --export on commas, so
# --export=ALL,POOLS=4E,4F,4G would pass POOLS=4E only.
