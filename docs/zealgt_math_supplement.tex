...
%% Models behind the ZEAL genotyping pipeline (docs/PLAN_pipeline_v2.md): the crossing scheme and the donor haplotype,
%% pooled samples (BC1 pools, BC2S3 bulks), per-donor discovery, the union of informative sites and gap filling, and the
%% two marker layers. Structure follows the nilHMM supplement (nilhmm-paper/supplement_engine_math.tex): numbered Texts,
...
\item[Text S6] The two marker layers and their checks
\end{description}
\clearpage
...
matters here, because the two gametes of a plant have different sources. This is the model of \code{build\_hd.py} (bzea-bc1-reference) and
of our step~4 (Text~\ref{sec:discovery}).
...
\label{eq:bulkmix}
\end{equation}
which keeps the $12.5\%$ donor content of Text~\ref{sec:scheme}. \todo{which generation is the bulk's parent: BC$_2$S$_2$ (used here) or BC$_2$S$_3$ (then $h=1/32$, $a=7/64$, B73 $=55/64$ in Eq.~\eqref{eq:bulkmix})} Within a segment where the parent is heterozygous, $K$ is constant between
...
With mean coverage $c$ per sample, the reads at a site are $\Pois(c)$ (Lander--Waterman). At the BC$_2$S$_3$ skim depths:

\begin{center}
\begin{tabular}{lcccc}
\toprule
$c$ & 0 reads & 1 read & 2 reads & $\ge 3$ reads\\
\midrule
...
The ALT reads at a site thin the Poisson: if the sample's ALT fraction is $f$, the ALT reads are $\Pois(cf)$ and the probability of seeing
none is $e^{-cf}$ (used in Eq.~\eqref{eq:veto}).
...
(5--25$\times$ each), where one carrier plant contributes about $c/12$ ALT reads, it can push weak sites over the thresholds. Duplicate
...
The veto keeps a candidate only if the witness pool shows $\ge1$ ALT read. For a real $\Hd$ allele, a line of depth $c$ shows no ALT read with
probability $E[e^{-c\,g}]$ over the bulk fraction $g$ of Eq.~\eqref{eq:bulkmix}; using
$E[e^{-cK/12}]=\big(\tfrac{1+e^{-c/12}}{2}\big)^{12}$,
\begin{equation}
q(c)=\tfrac{27}{32}+\tfrac{3}{32}e^{-c}+\tfrac{1}{16}\Big(\tfrac{1+e^{-c/12}}{2}\Big)^{12},
\qquad P(\text{real allele vetoed})\approx q(c)^{L}
\label{eq:veto}
\end{equation}
...
%======================================================================
\section{The two marker layers and their checks}
\label{sec:checks}
%======================================================================

\paragraph{Layer 1, ancestry.} Per line, RTIGER \citep{campos2023rtiger} on the donor's own tier-A sites (design BC$_2$S$_3$, rigidity 500)
...
\paragraph{Layer 2, genotypes.} Per donor, a two-founder PHG graph (B73 and the donor founder built from $D_{d\cdot}$ on $U$), imputing
...
the bulk mixture (Eq.~\eqref{eq:bulkmix}), with a Hotelling test on the (HET, ALT) fractions; this decides between the single-plant BC$_2$S$_3$
design and a BC$_2$S$_2$-parent design for the bulks. (ii) B73 controls free of donor segments. (iii) Segment-length distribution against the
BC$_2$S$_3$ expectation. \todo{derive the expected segment-length distribution of a bulk from Eq.~\eqref{eq:bulk}}
